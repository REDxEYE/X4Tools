...
class Buffer(abc.ABC, io.RawIOBase):
    ...
    def read_object(self, obj_class: Type['TR']) -> 'TR':
        return obj_class.from_buffer(self)
...
class WritableMemoryBuffer(io.BytesIO, Buffer):
    ...
    def slice(self, offset: Optional[int] = None, size: int = -1) -> 'MemorySlice':
        ...
        if size == -1:
            return MemorySlice(self.data[offset:], offset)
        return MemorySlice(self.data[offset:offset + size], offset)
